...
export default function Analytics() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gradient-to-br from-blue-900/40 to-blue-900/20 border border-blue-800/40 rounded-xl p-5 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-blue-500 to-blue-500" />
        <div className="flex gap-4 items-start relative z-10">
          <div className="w-10 h-10 rounded-full bg-blue-950/80 border border-blue-800 flex items-center justify-center flex-shrink-0">
            <Sparkles size={18} className="text-blue-400" />
          </div>
          <div>
            <h2 className="text-sm font-bold uppercase tracking-widest text-blue-300 mb-1">Copilot Insights</h2>
            {loadingInsight ? (
              <div className="animate-pulse flex gap-2 items-center text-sm text-blue-200/60 mt-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <KpiCard icon={Send}              label="Messages Sent"    value={sent.toLocaleString()}              color="blue" />
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
        ...
        {campaignPerf.length > 0 ? (
          <ResponsiveContainer width="100%" height={450}>
            <BarChart data={campaignPerf} layout="vertical" margin={{ left: 160, right: 30, top: 10, bottom: 10 }} barSize={18} barGap={4}>
              ...
              <Legend wrapperStyle={{ fontSize: 12, color: "#9ca3af", paddingTop: "10px" }} iconType="circle" />
              <Bar dataKey="open_rate"       name="Open Rate %"  fill="#0F62FE" radius={[0, 4, 4, 0]} />
              <Bar dataKey="ctr"             name="CTR %"        fill="#3b82f6" radius={[0, 4, 4, 0]} />
              <Bar dataKey="conversion_rate" name="Conv Rate %"  fill="#10b981" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        ) : (
            ...
            <tbody>
              {channelPerf.map((c) => (
                <tr key={c.channel} className="border-b border-gray-700/40 hover:bg-gray-700/20">
                  ...
                  <td className="py-2.5 pr-4">
                    <span className="text-blue-400 font-semibold">{parseFloat(c.open_rate || 0).toFixed(1)}%</span>
                  </td>
                  <td className="py-2.5 pr-4">
